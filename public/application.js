...
var populateBoard = function populateBoard(data) {
  // console.log('data is', data)
  clearBoard();
  var info = data.game.cells;
...
var createTable = function createTable(json) {
...
// GAME server
// CREATE success
var createSuccess = function createSuccess(createResponse) {
  ...
  if ($('#gameBoard').hasClass('playable')) {} else {
...
// UPDATE success
;var updateSuccess = function updateSuccess(updateResponse) {
...
// SHOW ALL error
var showAllError = function showAllError() {}
// console.log('showAllError is ', error)
// console.log('Failed to retrieve game data')


// SHOW ONE success
;var showOneSuccess = function showOneSuccess(showOneResponse) {
...
// SHOW ONE error
var showOneError = function showOneError() {}
// console.log('showOneError is ', error)
// console.log('Failed to retrieve game data')


// display message to sign in
;var mustSignIn = function mustSignIn() {
  $('#game-win-msg').html('Please sign in to play');
};

// make a move
var makeMoveSuccess = function makeMoveSuccess(box, game, num) {
  ...
  gameApi.updateGame().then(updateSuccess).catch(updateError);
};

var weHaveAWinner = function weHaveAWinner(value, num1, num2, num3) {
...
var weHaveADraw = function weHaveADraw() {
...
// SHOW ONE GAME
var showOneGame = function showOneGame(id) {
  // console.log('token is', store.user.token)
  return $.ajax({
    method: 'GET',
...
var authUi = __webpack_require__(11);
...
// on Sign In as guest
var onSignInGuest = function onSignInGuest(event) {
  event.preventDefault();
  ...
  var data = {
    ...
    // console.log('data is ', data)
  };authApi.signIn(data).then(authUi.signInSuccessGuest).catch(authUi.signInError);
};
...
var signInSuccessGuest = function signInSuccessGuest(signInGuestResponse) {
  clearMessages();
...
module.exports = {
  ...
  signInSuccessGuest: signInSuccessGuest
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
// SHOW contents of store.js
var onShowStore = function onShowStore() {
  console.log('store is ', store);
};
...
var onCreateGame = function onCreateGame(event) {
  ...
  // api
  gameApi.createGame().then(gameUi.createSuccess).catch(gameUi.createError);
};

// SHOW ALL GAMES from server
var onShowAllGames = function onShowAllGames(event) {
  // console.log('show all games was clicked')
  gameApi.showAllGames().then(gameUi.showAllSuccess).catch(gameUi.showAllError);
};

// SHOW ONE GAME from game list
var onShowClickedGame = function onShowClickedGame(event) {
  $('#modalGameList').modal('toggle');
  ...
  var gameID = element[0].textContent;
  // console.log('gameID is ', gameID)
  gameApi.showOneGame(gameID).then(gameUi.showOneSuccess).catch(gameUi.showOneError);
};

// SHOW ONE GAME from server
var onShowOneGame = function onShowOneGame(event) {
  event.preventDefault();
  // console.log('show one game was clicked')
  var data = getFormFields(event.target);
  // console.log('data.id is ', data.id)
  gameApi.showOneGame(data.id).then(gameUi.showOneSuccess).catch(gameUi.showOneError);
};

// GAME PLAY
// when box is clicked
var onSelectBox = function onSelectBox(event) {
  // stop if user is not signed in
  if (_typeof(store.user) === 'object') {} else {
    gameUi.mustSignIn();
    return;
  }
  ...
  if (store.hasOwnProperty('game')) {} else {
  ...
  if (store.game.over === true) {
    ...
    if ($('#current-player').hasClass('hide')) {} else {
      $('#current-player').toggleClass('hide');
    }
    return;
  }
  ...
  // win conditions
  var c = store.game.cells;
  // console.log('c is ', c)
  // console.log('num is ', num)

  var winCheck = function winCheck(c) {
    // win in rows
  ...
  // draw conditions
  var arrayFull = function arrayFull(value) {
    return value !== '';
  };
...
module.exports = {
  ...
  onShowAllGames: onShowAllGames,
  onShowClickedGame: onShowClickedGame,
  onShowOneGame: onShowOneGame,
